...
| Reference-data completeness | Submit an ingredient (and again via the correction path) from a supplier entry whose metadata carries no `verificationStatus`; confirm `batch` rejects with `reason: "missing_reference_metadata"` and writes no record — ADR-CT-033, `docs/14_developer_setup.md` §1.10 |
| Tamper evidence, demonstrated | Flip one byte of a record inside a proof bundle; confirm the verifier fails the record hash and the recomputed batch digest and exits non-zero — ADR-CT-034, `docs/14_developer_setup.md` §1.11 |
| Attestation verifiability | Verify a stored verdict attestation's ECDSA signature offline against the key `GetAttestationPublicKey` publishes; confirm an altered attestation and a signature from another key both fail — ADR-CT-034 |
| Evidence re-verification (T-006) | Retrieve a stored COA through its route; confirm the bytes re-hash to the value on the immutable record, and that a mismatched hash is refused (409) rather than served — ADR-CT-034 |
| Sandbox realism | Both Integrity Sandbox modes must return the deployed chaincode's own refusal text and leave the batch's trail unchanged — the property the canned version lacked (ADR-CT-034) |
