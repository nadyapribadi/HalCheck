...
- **v0.3.0:** Traced the accepted pre-build authority-closure contracts and their mandatory implementation tests.
...
| PRD-CT-005 | Fail recorded with equal permanence | FRD-CHAIN-LEDGER-004, VERDICT-005–007 | Architecture §9, DCG-002 | Batch SL-2026-002 (Seed Data §5), Verdict Detail flagged record | Enforcement Matrix — Fail handling, Flagged-record linkage, Verdict attestation | T-001, T-002 |
...
| PRD-CT-020 | Centralized System Audit Log | FRD-CHAIN-AUDIT-001, 004 | TRD §9/§13, Architecture §10, DCG-003 | AuditLogTable (Screen Req §13) | Audit availability gate | T-018 |
...
Every PRD-CT ID (001–023) has a traceable implementation owner. A `—` in the matrix denotes no distinct artifact is required beyond the adjacent controls, not an unreviewed gap. The five authority/data-model gaps are resolved by TRD §23; their listed acceptance tests remain mandatory implementation work. Concurrency requirements, while not tied to a single PRD-CT ID, are fully traced as a standalone row.
...
**Closed by TRD §23:** R-018 production-record correction linkage; controlled-vocabulary authority; verdict-attestation authority; audit-delivery completeness; and the canonical field-level RBAC matrix.
