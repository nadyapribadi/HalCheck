...
- **Resource-oriented, not screen-oriented:** every endpoint models a ledger resource and an action on it (a batch, a record, a reference entry) — never a specific frontend screen's data shape. Responses return complete, role-filtered records; reshaping data for a particular layout is the frontend's job, never the backend's. No endpoint, and no response field, may exist only because one screen currently needs it that way — a new or redesigned screen consumes an existing resource endpoint, it never justifies a new one. No server-side wizard or session state assumes a fixed step order, and no response ever carries a "next step" instruction — sequencing is a frontend routing decision, not a backend one. This is the concrete mechanism that keeps an unplanned frontend redesign or flow change from requiring a backend change: the API is stable because it was never shaped around today's screens to begin with.
